...
import { X, Play, ChevronLeft, ChevronRight, Youtube, Clapperboard } from 'lucide-react';

/**
 * Возвращает все трейлеры и тизеры с YouTube
 */
...
/**
 * Красивый модальный плеер трейлеров
 * Props: videos (объект TMDB videos), title, onClose
 */
export const TrailerModal = ({ videos, title, onClose }) => {
  ...
  // Сброс загрузки при смене трейлера
  useEffect(() => {
    setLoaded(false);
  }, [current]);
      ...
      >
        ...
        >
          ...
          <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-black shadow-2xl shadow-black/80">
            ...
            {!loaded && (
              <div className="absolute inset-0 flex items-center justify-center bg-black z-10">
                <div className="flex flex-col items-center gap-3">
                  <Clapperboard className="w-10 h-10 text-white/30 animate-pulse" />
            ...
            {/* Prev / Next навигация поверх плеера */}
            {trailers.length > 1 && (
              <>
                <button
                  onClick={prev}
          ...
          {/* Список трейлеров (если больше одного) */}
          {trailers.length > 1 && (
