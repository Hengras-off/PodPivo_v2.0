...
export const RussianVoicePlayer = ({ tmdbId, imdbId, title, year, mediaType, onClose }) => {
  ...
  const sources = [
    ...
    },
    {
      name: 'Rezka',
      getUrl: () => {
        if (imdbId) return `https://rezka.ag/embed/${imdbId}`;
        if (kinopoiskId) return `https://rezka.ag/embed/kp/${kinopoiskId}`;
        return null;
      },
      description: 'HDRezka — большой выбор озвучек',
      icon: '🇷🇺',
      quality: 'HD/Full HD',
      voiceovers: 'Русская озвучка'
    },
    {
      name: 'Kodik',
      getUrl: () => {
        if (kinopoiskId) return `https://kodik.info/film/${kinopoiskId}/embed`;
        if (imdbId) return `https://kodik.info/film/${imdbId}/embed`;
        return null;
      },
      description: 'Kodik — русская озвучка + аниме',
      icon: '🎭',
      quality: 'HD',
      voiceovers: 'Русская озвучка'
    },
    {
      name: 'Alloha',
      getUrl: () => {
        if (imdbId) return `https://sansa.newplayjj.com:9443/?imdb=${imdbId}`;
        if (kinopoiskId) return `https://sansa.newplayjj.com:9443/?kp=${kinopoiskId}`;
        return null;
      },
      description: 'Alloha — резервный источник',
      icon: '☁️',
      quality: 'HD/Full HD',
      voiceovers: 'Русская озвучка'
    },
    ...
    },
    {
      ...
      description: 'VidSrc Pro — альтернатива',
      icon: '🔥',
      quality: 'HD/Full HD',
      voiceovers: 'Субтитры EN'
    },
    {
      ...
      getUrl: () => {
        if (tmdbId) {
          const type = mediaType === 'movie' ? 'movie' : 'tv';
          return `https://multiembed.mov/directstream.php?video_id=${tmdbId}&tmdb=1&${type}=1`;
        }
        return null;
      },
      description: 'MultiEmbed — агрегатор',
      icon: '⚡',
      quality: 'HD',
