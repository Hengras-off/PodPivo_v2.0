...
export const RussianVoicePlayer = ({ tmdbId, imdbId, title, year, mediaType, onClose }) => {
  ...
  const sources = [
    ...
    },
    {
      name: 'Alloha',
      getUrl: () => {
        if (imdbId) return `https://apicollaps.cc/combine/embed?imdb_id=${imdbId}&translation_id=&token=`;
        if (kinopoiskId) return `https://apicollaps.cc/combine/embed?kp_id=${kinopoiskId}&token=`;
        return null;
      },
      description: 'Alloha — русская озвучка',
      icon: '☁️',
      quality: 'HD/Full HD',
      voiceovers: 'Русская озвучка'
    },
    {
      ...
      getUrl: () => {
        if (imdbId) {
          ...
          return `https://vidsrc-embed.ru/embed/${type}?imdb=${imdbId}`;
        }
        if (tmdbId) {
          const type = mediaType === 'movie' ? 'movie' : 'tv';
          return `https://vidsrc-embed.ru/embed/${type}?tmdb=${tmdbId}`;
        }
        return null;
      },
      description: 'Субтитры EN, без гео-блокировки',
      icon: '🌍',
      quality: 'HD/Full HD',
    ...
    },
    {
      ...
      description: 'VidSrc CC — альтернатива',
      icon: '🔥',
      quality: 'HD/Full HD',
      voiceovers: 'Субтитры EN'
    },
    {
      ...
      getUrl: () => {
        if (tmdbId) {
          const type = mediaType === 'movie' ? 'movie' : 'tv';
          return `https://vidsrc.icu/embed/${type}/${tmdbId}`;
        }
        return null;
      },
      description: 'VidSrc ICU — резервный',
      icon: '⚡',
      quality: 'HD',
      voiceovers: 'Субтитры EN'
    },
