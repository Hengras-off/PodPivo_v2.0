...
function AppInner() {
  ...
  // Пересоздаём Lenis при каждом переходе — гарантированный сброс скролла
  useEffect(() => {
    if (isPlayerPage) return;

    // Сначала уничтожаем старый
    if (lenisRef.current) {
  ...
  }, [location.pathname, location.hash, isPlayerPage]);
